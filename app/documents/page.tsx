...
export default function DocumentsPage() {
  ...
  return (
    <div className="p-4 md:p-8">
      {generating && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white/70 backdrop-blur-sm pointer-events-none">
          <div className="flex flex-col items-center gap-4">
            <img src="/logo.png" alt="PRO-LOGIC" className="h-10 object-contain animate-pulse" />
            <div className="w-48 h-1 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-gray-900 rounded-full" style={{ animation: 'pdfload 1.4s ease-in-out infinite', width: '40%' }} />
            </div>
            <p className="text-xs text-gray-500 tracking-wide">Generating document…</p>
          </div>
          <style>{`@keyframes pdfload{0%{transform:translateX(-100%)}50%{transform:translateX(150%)}100%{transform:translateX(-100%)}}`}</style>
        </div>
      )}
